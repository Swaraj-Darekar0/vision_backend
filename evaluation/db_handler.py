...
def write_session(user_id: str, scores: Dict,
    ...
    """
    Writes current session to Supabase session_scores table.
    ...
    try:
        ...
        if success:
            logger.info(f"Session {session_id} successfully written to Supabase.")
        return success
    except Exception as e:
        logger.error(f"Supabase write_session failed for {session_id}: {e}")
        return False
